...
CSDQsvEnc::CSDQsvEnc()
{
	...
	m_bEncClose = TRUE;
	m_pQsv264Enc = new CQsv264_Encoder;
...
BOOL CSDQsvEnc::CreateEncoder(h264_input_params* pInputParams, h264_enc_params* pEncParams)
{
	CSDMutex cs(m_pvEnc);
	if (m_bEncClose == FALSE)
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_WARNING, "Encode Open failed! Please close first!");
		mfCloseEncoder();
	}
	
	if ((pInputParams == NULL) || (pEncParams == NULL))
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "Encode Open failed! Point is NULL %p %p!", pInputParams, pEncParams);
		return FALSE;
	}
	...
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "Encode Open failed! Input format:%d invalid!", pInputParams->eInputFormat);
		return FALSE;
	}
	...
	//打开编码器
	Qsv264ExternParams tExternParamsHW;
	...
	bool bRet = m_pQsv264Enc->Open(pEncParams->unEncWidth, pEncParams->unEncHeight, pEncParams->unFramerate, 
									pEncParams->unBitrateKbps, pEncParams->unGopSeconds, &tExternParamsHW);
	if (bRet == false)
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "Encode Open failed with params:%dx%d  %dfps  %dkbps  gop:%d!",
	...
	m_pQsv264Enc->GetInputFormat(&nFormat, &nWidth, &nHeight);
	int nInputSize = av_image_get_buffer_size((AVPixelFormat)nFormat, nWidth, nHeight, 16);
	if ((nInputSize < 0) || (nFormat == AV_PIX_FMT_NONE))
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "av_image_get_buffer_size failed:%d! format:%d", nInputSize, nFormat);
		m_pQsv264Enc->Close();
		return FALSE;
	}
	...
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR ,"QsvH264 av_frame_alloc failed!");
		m_pQsv264Enc->Close();
		return FALSE;
	}
	...
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR ,"QsvH264 malloc failed with size:%d!", nInputSize);
	...
	if (bRet == false)
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "BmpZoom create failed with input:%dx%d format:%d output:%dx%d format:%d!", 
...
void CSDQsvEnc::CloseEncoder()
{
	CSDMutex cs(m_pvEnc);
	mfCloseEncoder();
}

void CSDQsvEnc::mfCloseEncoder()
{
	m_bEncClose = TRUE;
	m_pQsv264Enc->Close();
...
int CSDQsvEnc::Encode(const char* pImageData, char* pBitstreamData, int nMaxSupportLen)
{
	int nRet = 0;
	CSDMutex cs(m_pvEnc);
	if ((m_bEncClose == TRUE))
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "Encode fail, encoder is closed!");
		return nRet;
	}
	...
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "BmpZoom process failed!");
		return nRet;
	}
	...
	if (nRet > 0)
	{
		...
		nRet = m_pQsv264Enc->Encode(m_ptAVFrameEncode, (BYTE*)pBitstreamData, nMaxSupportLen, &nPts, &nDts);

	}
	else
	{
		SDLOG_PRINTF("CSDQsvEnc", SD_LOG_LEVEL_ERROR, "av_image_fill_arrays failed:%d!!!!", nRet);
	}

	return nRet;
}

void CSDQsvEnc::mfGetPresetString(video_enc_preset ePreset, char* strPreset)
{
	switch(ePreset)
	{
	case VIDEO_ENC_VERY_FAST:
		sprintf(strPreset, "veryfast");
		break;
	case VIDEO_ENC_FASTER:
		sprintf(strPreset, "faster");
		break;
	case VIDEO_ENC_FAST:
		sprintf(strPreset, "fast");
		break;
	case VIDEO_ENC_MEDIUM:
		sprintf(strPreset, "medium");
		break;
	case VIDEO_ENC_SLOW:
		sprintf(strPreset, "slow");
		break;
	case VIDEO_ENC_SLOWER:
		sprintf(strPreset, "slower");
		break;
	case VIDEO_ENC_VERY_SLOW:
		sprintf(strPreset, "veryslow");
		break;
	default:
